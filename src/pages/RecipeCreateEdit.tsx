...
type Category = 'Breakfast' | 'Lunch' | 'Dinner' | 'Brunch';
...
/** Small helper: textarea styled like your InputField */
...
export default function RecipeCreateEdit() {
    ...
    const validate = (data: FormState) => {
        ...
        if (!data.title.trim()) nextErrors.title = 'Recipe title required';
        if (!data.category) nextErrors.category = 'Category required';

        if (!data.preparationTime.trim()) nextErrors.preparationTime = 'Preparation time required';
        if (data.preparationTime && Number(data.preparationTime) <= 0) {
            nextErrors.preparationTime = 'Enter a valid time';
        }

        if (!data.people.trim()) nextErrors.people = 'No. people required';
        if (data.people && Number(data.people) <= 0) nextErrors.people = 'Enter a valid number';

        if (!data.shortDescription.trim())
            nextErrors.shortDescription = 'Short description required';
        if (!data.recipe.trim()) nextErrors.recipe = 'Recipe required';

        if (!data.recipeImage) {
            nextErrors.recipeImage = 'Recipe image is required';
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await postApi('/recipe/create-recipe', payload);
            if (response?.data) {
                    ...
                    <div className="w-full">
                        <form onSubmit={handleSubmit} className="w-full">
                            <div>
                                ...
                                {/* Save button (left aligned like image) */}
                                <div>
                                    <Button
